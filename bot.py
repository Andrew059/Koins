...
TOKEN="OTM0MTIwMzY5NDY2NTMxODUw.YerdYw.6peVAyKrtd3G1bz-L1KSbMw7mKI"
...
async def on_message(message):
    ...
    for i in range(len(message_database)):
        if str(message_database[i].keys()) == "dict_keys([" + str(message.author.id) + "])":
            if message_database[i][message.author.id] % 30 == 0: # For every 30 messages sent by the user, the user will receive one Koins
    ...
    if len(total_messages_sent) % 500 == 0: # For every 500 messages sent by users, each user will receive 30 Community Koins
        for i in range(len(message_database)):
    ...
    if split_message[0] == '$GiftKoins':
        ...
        if len(koins_database) == 0 and num8 == 0:
            ...
            if num != 1:
                ...
                num = 0
        if num8 == 0:
            await message.channel.send('```You have given ' + str(recipient_name) + ' ' + amount_of_money + ' Koins!```')
